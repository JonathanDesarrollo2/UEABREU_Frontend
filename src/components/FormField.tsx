...
type FormFieldProps<T extends FieldValues> = BaseFormFieldProps<T> & (
  | { type: 'select'; options: Array<{ value: string | number; text: string }> }
  ...
  <T extends FieldValues>(props: FormFieldProps<T>): React.JSX.Element => {
    ...
    const getFormattedValue = (): string | number => {
      ...
      if (type === 'select' && options) {
        ...
        // Buscar si existe la opción (comparación flexible string/number)
        const optionExists = options.some(
          opt => opt.value.toString() === stringValue
        );
        return optionExists ? stringValue : '';
      }
    ...
    const secureRegister = (
      ...
      const { onChange, ...rest } = register(name, {
        ...registerOptions,
        setValueAs: (value) => {
          // Si el tipo es 'select' y las opciones son numéricas, convertir a número
    ...
    if (type === 'select') {
      // Determinar si las opciones son numéricas para manejar defaultValue correctamente
      const areOptionsNumeric = options && options.length > 0 && typeof options[0].value === 'number';
      // Para defaultValue, si es número y las opciones son numéricas, usar el valor numérico directamente
      const selectDefaultValue = areOptionsNumeric && typeof defaultValue === 'number'
        ? defaultValue
        : formattedValue;
